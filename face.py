...
haar_faces = cv2.CascadeClassifier(serverConfig.HAAR_FACES)
...
def detect_single(image):
	...
	faces = haar_faces.detectMultiScale(image, 
				scaleFactor=serverConfig.HAAR_SCALE_FACTOR,
				minNeighbors=serverConfig.HAAR_MIN_NEIGHBORS,
				minSize=serverConfig.HAAR_MIN_SIZE,
...
def crop(image, x, y, w, h):
	...
	"""
	crop_height = int((serverConfig.FACE_HEIGHT / float(serverConfig.FACE_WIDTH)) * w)
	midy = y + h/2
...
def resize(image):
	...
	return cv2.resize(image, 
					  (serverConfig.FACE_WIDTH, serverConfig.FACE_HEIGHT),
